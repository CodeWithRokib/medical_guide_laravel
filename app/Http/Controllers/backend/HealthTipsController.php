...
class HealthTipsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.health-tips.add',compact('healthtips'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($image) {
            ...
            $image->move(base_path('public/admin/health-tips/upload/'), $filename);
            $data          = $request->except('image');
            $data['image'] = $filename;
            HealthTips::create($data);
        }else{
            HealthTips::create($request->all());
        }
        session()->flash('success','Health Tips  saved');
        return redirect()->route('healthtips.add');
    }

    public function show($id)
    {
        $healthtips = HealthTips::find($id);
        return view('admin.health-tips.edit',compact('healthtips'));
    }
...
//            'name' => [ 'required', Rule::unique('hospitals')->ignore($request->id)],
    ...
    public function destroy(Request $request)
    {
        ...
        /* Image Unlink Start */
        $img_path = HealthTips::find($request->id);
        $path = 'public/admin/health-tips/upload/'.$img_path->image;
        unlink($path);
        /* Image Unlink End */
        $img_path->delete();
    }
